...
import { ACCOUNTS, BANKS, CATS, PEOPLE, RULES, type Person } from "@/lib/sample/data";
...
function KisilerTab() {
  ...
  const add = () => {
    ...
    setPeople([
      ...people,
      { id, name: name.trim(), role: "self", color: COLORS[people.length % COLORS.length] },
    ]);
    setName("");
  };
    ...
    <div className="grid-base" style={{ gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "start" }}>
      <div className="card">
        <div className="card-head">
          ...
          <div className="card-sub">{people.length} kişi</div>
        </div>
        <div
          style={{
            padding: "10px 14px",
        ...
        >
          ...
          />
          <button className="btn btn-prim" onClick={add}><Icon name="plus" size={12} /> Ekle</button>
        </div>
        <div>
          {people.map((p, i) => (
            <div
            ...
            >
              <div className="avatar" style={{ background: p.color, color: "#0a0d14", width: 28, height: 28, fontSize: 11 }}>
                {p.name[0]}
              </div>
              <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 4 }}>
                {COLORS.map((c) => (
                  <button
                    key={c}
                    onClick={() => setPeople(people.map((x) => (x.id === p.id ? { ...x, color: c } : x)))}
                    style={{
                      width: 14,
                      height: 14,
                      borderRadius: 50,
              ...
              <button className="icon-btn" onClick={() => setPeople(people.filter((x) => x.id !== p.id))}>
      ...
      <div className="card">
        ...
        <div className="card-pad" style={{ fontSize: 13, color: "var(--fg-soft)", lineHeight: 1.6 }}>
          ...
          <p style={{ marginTop: 14 }}>Renk seçimi her ekrandaki chip/donut&apos;larda kullanılır. Daha sonra istediğinde değiştirebilirsin.</p>
        </div>
      </div>
    </div>
  );
